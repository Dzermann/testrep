...
def extract_column_from_header(th_element):
    return th_element.text.strip() if th_element is not None else None
header_elements = first_launch_table.find_all('th')
column_names = [extract_column_from_header(th) for th in header_elements if th and th.text.strip()]
print(column_names)
